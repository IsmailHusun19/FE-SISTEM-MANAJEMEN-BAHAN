...
const CetakLaporanDetailBahanMasuk = forwardRef(({ bahanMasuk }, ref) => {
  const totalQty =
    bahanMasuk?.bahanMasuk?.reduce((a, b) => a + b.jumlah, 0) || 0;
    ...
    >
          ...
          <div className="text-right text-[10px] space-y-1">
            ...
            <div className="inline-flex items-center gap-1 border px-2 py-1">
              ...
              <span className="font-semibold">
                {bahanMasuk?.konfirmasi ? "APPROVED" : "DRAFT"}
              </span>
            </div>
            ...
            <div className="text-slate-500 mt-2">
              ...
              <div>
                DATE:
                <span className="ml-1 font-semibold text-slate-800">
                  {new Date(bahanMasuk?.tanggalMasuk).toLocaleDateString("id-ID")}
                </span>
              </div>
            </div>
      ...
      <div className="mb-5">
        <h2 className="text-[14px] font-bold tracking-tight">
          MATERIAL INBOUND REPORT
        </h2>
        <p className="text-[10px] text-slate-500">
          Official warehouse receiving document
        </p>
      </div>
...
<div className="border border-slate-200 rounded-xl overflow-hidden mb-6 text-[10px]">
  ...
  <div className="bg-slate-50 px-4 py-2 border-b border-slate-200 flex justify-between items-center">
    ...
    <div className={`px-2 py-1 rounded-md text-[9px] font-bold tracking-wider
      ${bahanMasuk?.konfirmasi
        ? "bg-emerald-100 text-emerald-700"
        : "bg-orange-100 text-orange-700"
      }`}
    >
      {bahanMasuk?.konfirmasi ? "APPROVED" : "PENDING"}
    </div>
    ...
    <div className="px-4 py-3">
      ...
      <div className="flex gap-3 text-[11px] font-semibold">
        ...
        <span className="text-slate-800">
          {bahanMasuk?.bahanMasuk?.length || 0} Items
        </span>
  ...
  <div className="border-t border-slate-200 bg-white px-4 py-3">
    ...
    <div className="text-[11px] text-slate-700 leading-relaxed">
      {bahanMasuk?.catatan || "No additional notes provided."}
    </div>
      ...
      <div className="border border-slate-300">
        ...
        <table className="w-full text-[11px]">
          ...
          <tbody>
            {bahanMasuk?.bahanMasuk?.map((item, i) => (
...
export default CetakLaporanDetailBahanMasuk;