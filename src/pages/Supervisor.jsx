...
const Supervisor = () => {
  ...
  const getData = async () => {
    try {
      ...
      const supervisorProduksi = getUsers.data
      .filter(
        (user) =>
      ...
      setSupervisor(supervisorProduksi);
    } catch (error) {
  ...
  const handleAdd = () => {
    navigate("/pengguna/tambah-supervisor");
  };
  const handleEdit = (id) => {
    navigate(`/pengguna/edit-supervisor/${id}`);
  };
  ...
  const filteredSupervisor = supervisor.filter((item) =>
    item.name.toLowerCase().includes(search.toLowerCase())
  );

  const totalItems = filteredSupervisor.length;
  const totalPages = Math.ceil(totalItems / ITEMS_PER_PAGE);
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const endIndex = startIndex + ITEMS_PER_PAGE;
  const currentSupervisor = filteredSupervisor.slice(startIndex, endIndex);
  useEffect(() => {
    setCurrentPage(1);
  }, [search]);
        ...
        >
          ...
          <div className="px-4 sm:px-6 lg:px-8 py-6">
            ...
            >
              ...
              <div>
                ...
                <h1 className="text-[30px] sm:text-[42px] leading-none font-bold text-slate-900 mt-2">
                  Data <span className="text-orange-500">Supervisor</span>
                </h1>

                <p className="text-slate-500 text-sm mt-3 max-w-xl">
                Kelola data supervisor produksi yang bertanggung jawab dalam proses operasional.
                </p>
              </div>
            ...
            <div className="w-full flex justify-end mt-4 lg:mt-0">
              <button
                onClick={() => navigate("/pengguna/supervisor-nonaktif")}
                className="
    h-11 px-5
    rounded-2xl
    mb-4
              ...
              <div className="block lg:hidden">
                {currentSupervisor.length > 0 ? (
                  <div className="divide-y divide-[#f3f3f3]">
                    {currentSupervisor.map((item, index) => (
      ...
      >
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="text-sm font-semibold text-slate-600">
            Data supervisor kosong
          </p>

          <p className="text-xs text-slate-400 mt-1">
            Belum ada data supervisor produksi
          </p>
        </div>
      </td>
    </tr>
