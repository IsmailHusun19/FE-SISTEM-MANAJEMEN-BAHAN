...
const Leader = () => {
  ...
  const getData = async () => {
    try {
      ...
      const LeaderProduksi = getUsers.data
      .filter(
        (user) =>
      ...
      setLeader(LeaderProduksi);
    } catch (error) {
      console.error("Gagal mengambil data leader produksi :", error);
      setLeader([]);
    }
  };
  ...
  const handleAdd = () => {
    navigate("/pengguna/tambah-Leader");
  };
  const handleEdit = (id) => {
    navigate(`/pengguna/edit-Leader/${id}`);
  };
  ...
  const filteredLeader = leader.filter((item) =>
    item.name.toLowerCase().includes(search.toLowerCase())
  );

  const totalItems = filteredLeader.length;
  const totalPages = Math.ceil(totalItems / ITEMS_PER_PAGE);
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const endIndex = startIndex + ITEMS_PER_PAGE;
  const currentLeader = filteredLeader.slice(startIndex, endIndex);
  useEffect(() => {
    setCurrentPage(1);
  }, [search]);
            ...
            >
              ...
              <div>
                ...
                <h1 className="text-[30px] sm:text-[42px] leading-none font-bold text-slate-900 mt-2">
                  Data <span className="text-orange-500">Leader Produksi</span>
                </h1>

                <p className="text-slate-500 text-sm mt-3 max-w-xl">
                Kelola data leader produksi yang bertanggung jawab dalam pengawasan, koordinasi, dan operasional proses produksi.
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto">
                ...
                <div className="w-full sm:w-[320px] relative">
                  ...
                  <input
                    ...
                    placeholder="Cari leader produksi..."
                    className="
                      w-full h-11
                      rounded-2xl
                ...
                >
                  <Plus size={18} />
                  Tambah Leader Produksi
                </button>
              </div>
            </div>
            <div className="w-full flex justify-end mt-4 lg:mt-0">
              <button
                onClick={() => navigate("/pengguna/leader-nonaktif")}
                className="
    h-11 px-5
    rounded-2xl
    mb-4
  ...
  "
            ...
            >
              ...
              <div className="block lg:hidden">
                {currentLeader.length > 0 ? (
                  <div className="divide-y divide-[#f3f3f3]">
                    {currentLeader.map((item, index) => (
      ...
      >
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="text-sm font-semibold text-slate-600">
            Data leader produksi kosong
          </p>

          <p className="text-xs text-slate-400 mt-1">
            Belum ada data leader produksi
          </p>
        </div>
      </td>
    </tr>
