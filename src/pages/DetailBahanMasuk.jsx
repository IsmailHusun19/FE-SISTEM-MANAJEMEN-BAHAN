...
import {
  ...
  DeleteBahanMasuk,
  EditStatusKonfirmasiBahanMasuk,
} from "../service/Api";
import {
  Inbox,
  Printer,
...
import CetakLaporanDetailBahanMasuk from "../component/CetakLaporanDetailBahanMasuk";
import { toast } from "sonner";

const DetailPenerimaanBahan = () => {
  const navigate = useNavigate();
  const { expanded } = useContext(SidebarContext);
  const { user } = useAuth();
  const [bahanMasuk, setBahanMasuk] = useState(null);
  const { id } = useParams();
  ...
  const getData = async () => {
    try {
      setLoading(true);
      const item = await GetDetailPenerimaanBahan(id);
      setBahanMasuk(item.data);
      console.log(item.data);
    } catch (error) {
      console.error("Gagal mengambil data staff Bahan :", error);
      setBahanMasuk([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (id) => {
    ...
    toast("Konfirmasi Penghapusan", {
      description: "Data dihapus permanen",
      position: "top-center",
      duration: Infinity,
  ...
  const handleEdit = (id) => {
    navigate(`/bahan/edit-penerimaan-bahan/${id}`);
  };

  const handleKonfirmasi = async (id, currentStatus) => {
    if (!id) return;

    const isKonfirmasi = !currentStatus;

    toast(
      isKonfirmasi ? "Konfirmasi Penerimaan Bahan" : "Batalkan Konfirmasi",
      {
        description: isKonfirmasi
          ? "Stok bahan akan ditambahkan ke gudang"
          : "Stok bahan akan dikurangi kembali dari gudang",
        ...
        className: `
          !rounded-2xl
          !border ${isKonfirmasi ? "!border-emerald-100" : "!border-orange-100"}
          !bg-white/90
          !backdrop-blur-xl
          !text-slate-800
        ...
        action: {
          label: isKonfirmasi ? "Konfirmasi" : "Hapus Validasi",
          ...
          onClick: async () => {
            const loadingToast = toast.loading(
                ...
                : "Membatalkan validasi...",
              {
                ...
                className: `
                  !rounded-2xl
                  !border ${
                    isKonfirmasi ? "!border-emerald-100" : "!border-orange-100"
                  }
                  !bg-white
                  !text-slate-800
            ...
            try {
              await EditStatusKonfirmasiBahanMasuk(id, {
                konfirmasi: isKonfirmasi,
              });

              toast.success(
                isKonfirmasi
                  ? "Transaksi berhasil dikonfirmasi"
                  : "Konfirmasi berhasil dibatalkan",
                {
                  id: loadingToast,
                  position: "top-right",
                  ...
                  description: isKonfirmasi
                    ...
                    : "Stok bahan berhasil dikurangi kembali",
              ...
              toast.error(
                isKonfirmasi
                  ? "Gagal mengkonfirmasi transaksi"
                  : "Gagal membatalkan konfirmasi",
                {
                  id: loadingToast,
                  position: "top-right",
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `Penerimaan-${bahanMasuk?.id}`,
  });
  ...
  return (
                  ...
                  <div className="relative z-10 flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
                    ...
                    "
                      ...
                      <p className="mt-2 text-sm text-slate-500 leading-relaxed max-w-2xl">
                        Informasi lengkap transaksi penerimaan bahan produksi.
                      </p>
                    </div>
    ...
    `}
                          ...
                          <div className="flex items-center gap-2">
                            {!bahanMasuk.konfirmasi && (
                              <>
                                {/* EDIT */}
                                <button
                                  onClick={() => handleEdit(bahanMasuk.id)}
                                  className="
        h-9 px-3.5
        rounded-xl
        border border-orange-100
      ...
      "
                                ...
                                {/* HAPUS */}
                                <button
                                  onClick={() => handleDelete(bahanMasuk.id)}
                                  className="
        h-9 px-3.5
        rounded-xl
        border border-red-100
  ...
  `}
                  ...
                  "
                    >
                      ...
                      <div className="flex items-center justify-between mb-5">
                        <div>
                          ...
                          <p className="text-sm text-slate-400 mt-1">
                            Seluruh bahan yang diterima pada transaksi ini.
                          </p>
                        </div>
                      ...
                      "
                        >
                          {bahanMasuk.bahanMasuk?.length || 0} ITEM
                        </div>
                      </div>

                      {/* LIST */}
                      <div className="space-y-4">
                        {bahanMasuk.bahanMasuk?.map((detail, detailIndex) => (
                          <div
                            key={detail.id}
                            className="
                            rounded-[24px]
                              ...
                              <div className="flex flex-wrap items-center gap-3">
                                ...
                                >
                                  ...
                                  <h4 className="mt-1 text-sm font-bold text-orange-600">
                                    {detail.jumlah} {detail.bahan?.satuan}
                                  </h4>
                                </div>
                                ...
                                >
                                  ...
                                  <h4 className="mt-1 text-sm font-bold text-slate-700">
                                    {detail.bahan?.stok} {detail.bahan?.satuan}
                                  </h4>
                                </div>
                              </div>
                            </div>
                  ...
                  "
                    >
                      ...
                      <p className="text-sm text-slate-400 mt-1">
                        Statistik singkat penerimaan bahan.
                      </p>
                      ...
                      "
                        >
                          ...
                          <h2 className="mt-2 text-2xl font-bold text-slate-800">
                            {bahanMasuk.bahanMasuk?.length || 0}
                          </h2>
                        </div>
                      ...
                      "
                        >
                          ...
                          <h2 className="mt-2 text-sm font-bold text-slate-700 leading-relaxed">
                            {bahanMasuk.supplier}
                          </h2>
                        </div>
                      </div>
      ...
      <div className="hidden">
        <CetakLaporanDetailBahanMasuk ref={printRef} bahanMasuk={bahanMasuk} />
      </div>
    </>
  );
};

export default DetailPenerimaanBahan;
